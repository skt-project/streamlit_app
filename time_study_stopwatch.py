...
def _load_from_streamlit_secrets(key: str) -> Tuple:
    ...
    return creds, d.get("project_id") or d.get("project") or Config.BQ_PROJECT
...
def fetch_gps() -> Tuple[Optional[float], Optional[float], Optional[int]]:
    """
    Call streamlit-js-eval's get_geolocation() which runs in the main page
    context (NOT an iframe) so browser GPS permission works correctly.
    Returns (lat, lng, accuracy_m) or (None, None, None).
    """
    try:
        loc = get_geolocation()   # blocks until browser resolves
        if loc and "coords" in loc:
            ...
            return (
                ...
                int(acc)   if acc is not None else None,
            )
    except Exception as e:
        logger.warning(f"GPS fetch error: {e}")
...
def _make_event_id(spv, activity_key, store_id, logged_at) -> str:
    ts = logged_at.isoformat() if isinstance(logged_at, datetime) else str(logged_at or "")
    return hashlib.sha256(f"{spv}|{activity_key}|{store_id or ''}|{ts}".encode()).hexdigest()
...
def _dedup(client, event_ids: List[str]) -> set:
    if not event_ids:
        return set()
...
def _insert(client, rows: list) -> Tuple[bool, str]:
    ...
    try:
        errors = client.insert_rows_json(
            ...
            [_serialize(r) for r in new_rows],
            row_ids=[r["event_id"] for r in new_rows],
        )
        if errors:
            return False, f"BQ errors: {errors}"
        msg = f"✅ {len(new_rows)} record tersimpan."
        if skipped:
            msg += f" ({skipped} duplikat dilewati)"
        return True, msg
    except Exception as e:
        logger.error(f"BQ insert: {e}")
        return False, str(e)


def write_p1(spv, region, distributor, entry, lat=None, lng=None, acc=None) -> Tuple[bool, str]:
    client = get_bq_client()
    if not client:
    ...
    row = {
        ...
        "spv": spv, "region": region, "distributor": distributor,
    ...
    return _insert(client, [row])


def write_session(spv, region, distributor, entry) -> Tuple[bool, str]:
    client = get_bq_client()
    if not client:
        return False, "No BQ client."
    duration_s = int(entry.get("duration_ms", 0) / 1000)
...
def init_state():
    defaults = {
        ...
        "store_id": "", "store_name": "",
        "store_geo_done": set(),   # stores whose first-activity GPS is captured
        "totals": {},
        # GPS capture flags
        "capture_dist_in_geo": False,    # trigger GPS for dist_in
        "pending_dist_in": None,         # entry dict waiting for GPS
        "capture_store_geo": False,      # trigger GPS for first store activity
        "pending_store_session": None,   # session dict waiting for GPS
    }
...
def maybe_capture_dist_in_geo():
    """
    If dist_in GPS capture is pending, fetch GPS here (top of render, before
    any other widgets so get_geolocation is called unconditionally) then write
    to BQ and clear the flag.
    """
    if not st.session_state.capture_dist_in_geo:
        return False   # not pending

    entry = st.session_state.pending_dist_in
    st.info("📡 Mengakses GPS lokasi distributor… (izinkan akses lokasi di browser)")

    with st.spinner("Menunggu GPS…"):
        lat, lng, acc = fetch_gps()

    st.session_state.capture_dist_in_geo = False
    ...
    if ok:
        ...
        geo = f"📍 {lat:.5f}, {lng:.5f} ±{acc}m" if lat else "📍 lokasi tidak tersedia"
        st.success(f"📥 **Check In Distributor** disimpan · {geo}")
    else:
...
def maybe_capture_store_geo():
    """
    If first-store-activity GPS capture is pending, fetch GPS then write to BQ.
    """
    if not st.session_state.capture_store_geo:
        return False

    entry = st.session_state.pending_store_session
    st.info("📡 Mengakses GPS lokasi toko… (izinkan akses lokasi di browser)")

    with st.spinner("Menunggu GPS…"):
        lat, lng, acc = fetch_gps()

    st.session_state.capture_store_geo      = False
    ...
    with st.spinner("Menyimpan ke BigQuery…"):
        ok, msg = write_session(
            st.session_state.spv, st.session_state.region,
            st.session_state.distributor, entry,
        )
    if ok:
        geo = f"📍 {lat:.5f}, {lng:.5f} ±{acc}m" if lat else "📍 lokasi tidak tersedia"
        st.success(f"✅ **{entry['activity_label']}** — {fmt_ms(entry['duration_ms'])} · {geo}")
    else:
        st.error(f"❌ Gagal: {msg}")
...
def render_setup():
    ...
    distributor = st.selectbox("🏭 Distributor", [""] + dist_list, key="sel_dist", disabled=not region)

    st.markdown("")
    if st.button("Mulai Kunjungan →", type="primary", disabled=not (spv and region and distributor),
...
def render_checkin():
    # ── GPS capture intercepts — must come before any other UI ──
    consumed_p1 = maybe_capture_dist_in_geo()
    consumed_p2 = maybe_capture_store_geo()
    if consumed_p1 or consumed_p2:
        st.divider()
        if st.button("← Lanjutkan", use_container_width=True):
            st.rerun()
        return   # show result only; user taps to continue
...
def _render_p1_tab():
    ...
    chosen_label = st.selectbox("Pilih Aktivitas",
                                ["— Pilih Aktivitas —"] + list(act_options.keys()), key="p1_sel")
    ...
    if st.button("✅ Catat Waktu Sekarang", type="primary", use_container_width=True):
        if chosen_label == "— Pilih Aktivitas —":
            ...
            if key == "dist_in":
                # Defer to GPS capture screen
                st.session_state.pending_dist_in      = entry
                st.session_state.capture_dist_in_geo  = True
                st.rerun()
            else:
                # All other P1 actions: write immediately, no GPS
...
def _render_p2_tab():
    ...
    if st.session_state.act_key:
        ...
        if a:
            cur_act_label = f"{a['icon']} {a['label']}"

    act_choice = st.selectbox("Pilih Aktivitas", act_list,
                              index=(act_list.index(cur_act_label) if cur_act_label in act_list else 0),
                              key="p2_act_sel")
    if act_choice != "— Pilih Aktivitas —":
        new_key = act_key_map[act_choice]
        if new_key != st.session_state.act_key:
            if st.session_state.timer_running:
                st.session_state.timer_elapsed_ms = get_live_ms()
                st.session_state.timer_running    = False
                st.session_state.timer_started_at = None
            ...
            st.session_state.act_label = f"{a['icon']} {a['label']}"
    ...
    status_txt = "🟢 Recording" if st.session_state.timer_running else ("⏸ Paused" if elapsed > 0 else "○ Idle")
    ...
    <div style="background:#12151c;border:1px solid rgba(255,255,255,0.08);border-radius:14px;
      ...
      <div style="font-size:0.72rem;color:#6b7280;margin-top:8px;letter-spacing:1px;
                  text-transform:uppercase;">{status_txt}</div>
    </div>
    """, unsafe_allow_html=True)
    ...
    for i, a in enumerate(ACTIVITIES):
        ...
        if st.session_state.act_key == a["key"]:
            total += live
        with cols[i % 4]:
            st.metric(f"{a['icon']} {a['label']}", fmt_ms(total))


def _do_stop_and_save():
    elapsed  = get_live_ms()
    ...
    entry = {
        "activity_key":        st.session_state.act_key,
        "activity_label":      st.session_state.act_label,
        "store_id":            store_id,
        "store_name":          st.session_state.store_name or "—",
        "duration_ms":         elapsed,
    ...
    if store_id and store_id not in st.session_state.store_geo_done:
        # First activity for this store — capture GPS before writing
        st.session_state.pending_store_session = entry
        st.session_state.capture_store_geo     = True
        st.rerun()
    else:
        # GPS already on record — write immediately
        with st.spinner("Menyimpan…"):

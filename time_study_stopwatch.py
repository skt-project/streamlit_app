...
def init_state():
    defaults = {
        ...
        "store_geo_done": set(),
        "totals": {},
        # GPS action queues — set to True to trigger a GPS-then-write on next render
...
# ============================================================
# MAIN — get_geolocation() called here UNCONDITIONALLY every render
# ============================================================
def main():
    init_state()
...
def _render_p1_tab():
    ...
    if st.button("✅ Catat Waktu", type="primary", use_container_width=True):
        if chosen == "— Pilih Aktivitas —":
            ...
            if key == "dist_in":
                # Queue GPS capture — will be processed at top of next render

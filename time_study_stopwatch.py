...
BQ_QUERY = """
    SELECT
        ...
        CASE WHEN region_g2g = '' THEN upper(region) ELSE upper(region_g2g) END AS region,
...
def _load_secrets(key):
    ...
    return (
        service_account.Credentials.from_service_account_info(d),
...
def _load_local():
    ...
    return (
        service_account.Credentials.from_service_account_file(Config.CREDENTIALS_PATH),
        Config.BQ_PROJECT,
    )
...
def _insert(client, rows):
    ...
    try:
        errs = client.insert_rows_json(
            ...
            [_ser(r) for r in new],
            row_ids=[r["event_id"] for r in new],
        )
        if errs:
...
def write_p1(spv, region, dist, entry, lat=None, lng=None, acc=None):
    ...
    row = {
        "event_id":            _eid(spv, entry["key"], None, logged_at),
        "spv":                 spv,
        "region":              region,
        "distributor":         dist,
        "activity_key":        entry["key"],
        "activity_label":      entry["label"],
        "logged_at":           logged_at,
        "store_id":            None,
        "store_name":          None,
...
def write_session(spv, region, dist, entry):
    ...
    row = {
        "event_id":            _eid(spv, entry["activity_key"], sid, ended),
        "spv":                 spv,
        "region":              region,
        "distributor":         dist,
...
def main():
    ...
    # ----------------------------------------------------------
    # Write Phase Handler (Controlled Geolocation Trigger)
    # ----------------------------------------------------------
    if st.session_state.write_phase is not None:
        loc = get_geolocation()

        if loc is None:
            st.info("📡 Mengambil koordinat GPS… pastikan izin lokasi diaktifkan.")
            return  # Stop execution until GPS arrives

        lat, lng, acc = _extract_coords(loc)
        payload = st.session_state.pending_payload
        phase   = st.session_state.write_phase

        # Reset state BEFORE writing to prevent re-trigger loop
        ...
        if phase == "dist_in":
            ...
            payload["location_accuracy_m"] = acc
            ...
            if ok:
                ...
                st.success(
                    ...
                    f"{fmt_ms(payload['duration_ms'])} · {geo_str}"
                )
            else:
                st.error(f"❌ Gagal: {msg}")
    ...
    # ----------------------------------------------------------
    # Normal App Flow (no GPS calls here)
    # ----------------------------------------------------------
...
def render_setup():
    ...
    region_list = (
        master.get("by_spv", {}).get(spv, {}).get("regions", []) if spv else []
    )
    with c2:
        region = st.selectbox(
...
def render_checkin():
    ...
    with c2:
        st.info(
            f"**{st.session_state.spv}**  \n"
            f"{st.session_state.distributor} · {st.session_state.region}"
        )
...
def _render_p1_tab():
    ...
    chosen   = st.selectbox(
        "Pilih Aktivitas",
        ["— Pilih Aktivitas —"] + list(act_opts.keys()),
        key="p1_sel",
    )
    ...
    if st.button("✅ Catat Waktu", type="primary", use_container_width=True):
        if chosen == "— Pilih Aktivitas —":
            ...
            if key == "dist_in":
                st.session_state.pending_payload = entry
...
def _render_p2_tab():
    ...
    cur_label  = next(
        (k for k, v in store_opts.items() if v["store_id"] == st.session_state.store_id),
        "",
    )
    ...
    if st.session_state.store_id:
        ...
        with col_geo:
            if st.session_state.store_id in st.session_state.store_geo_done:
                st.markdown(
                    '<span class="geo-ok">📍 Lokasi toko sudah terekam</span>',
                    unsafe_allow_html=True,
                )
            else:
                st.markdown(
                    '<span class="geo-warn">📍 GPS direkam pada aktivitas pertama</span>',
                    unsafe_allow_html=True,
                )
        with col_reset:
            if st.button(
    ...
    clr      = (
        "#4ade80" if st.session_state.timer_running
        else ("#f5a623" if elapsed > 0 else "#e8eaf0")
    )
    status   = (
        "🟢 Recording" if st.session_state.timer_running
        else ("🟡 Ready" if elapsed > 0 else "○ Idle")
    )
...
def _do_stop():
    ...
    if store_id and store_id not in st.session_state.store_geo_done:
        # Capture GPS on the first session for this store
        st.session_state.pending_payload = entry

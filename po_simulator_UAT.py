...
def _logo_src() -> str:
    ...
    return "https://glad2glow.com/cdn/shop/files/logo-svg.svg?v=1777280269&width=280"
...
with st.sidebar:
    ...
    #"G2G-30701","G2G-30702","G2G-30703","G2G-30704",
...
with tabs[0]:
    ...
    def excel_to_pdf_libreoffice(xlsx_bytes: bytes) -> bytes:
        """Convert xlsx bytes ke PDF pakai LibreOffice headless, hasil identik dgn tampilan Excel."""
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            if not os.path.exists(pdf_path):
                raise RuntimeError(f"LibreOffice gagal convert: {result.stderr.decode(errors='ignore')}")
    ...
    if 'export_bytes' in st.session_state:
        ...
        with dl_col2:
            try:
                pdf_bytes = excel_to_pdf_libreoffice(st.session_state['export_bytes'])
                ...
                st.error(f"Gagal generate PDF: {e}. Pastikan LibreOffice sudah terinstall di server (soffice command).")

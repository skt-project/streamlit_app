...
/* ── Styled containers: meta cards & question cards ── */
...
/* ── Question cards (override inner border-left) ── */
...
/* ── Primary buttons (Review/Submit/Confirm/Login) ── */
...
/* ── Mobile-friendly bigger option text & touch targets ── */
...
/* ── Recolor radio checked dot from theme pink to brand blue ── */
label[data-baseweb="radio"] > div:has(~ input:checked) {
    background-color: #1E6B8A !important;
}

/* ── Recolor progress bar fill from theme pink to brand blue ── */
[data-testid="stProgressBarTrack"] > div {
    background-color: #1E6B8A !important;
}

/* ── Streamlit label weight & size ── */
...
def load_master_distributor():
    return pd.DataFrame({
        "region": ["Jakarta","Jakarta","Jakarta",
                   "West Java","West Java",
                   "East Java","East Java",
                   "North Sumatra","North Sumatra",
                   "South Sumatra"],
        "distributor_code": [f"D{i:03d}" for i in range(1, 11)],
        "distributor": [
            "CV Maju Bersama - Jakarta Pusat",
            "PT Sinar Jaya - Jakarta Utara",
            "PT Sinar Jaya - Jakarta Selatan",
            "CV Mitra Makmur - Bandung",
            "CV Mitra Makmur - Bekasi",
            "PT Cahaya Timur - Surabaya",
            "PT Cahaya Timur - Malang",
            "CV Berkah Mandiri - Medan",
            "CV Berkah Mandiri - Deli Serdang",
            "PT Surya Selatan - Palembang",
        ]
    })
...
MOCK_YTD = {
    ...
    "PT Cahaya Timur - Malang":           2_100_000_000,
    "CV Berkah Mandiri - Medan":          1_800_000_000,
    "CV Berkah Mandiri - Deli Serdang":   1_200_000_000,
...
# =====================================================
# LOGIN — username/password mapped 1:1 to a role (MOCK ONLY)
# Plain password per request (simpler for a small internal team). The real
# app should back this with the assessment_users BigQuery table instead of
# a hardcoded dict.
# =====================================================
MOCK_USERS = {
    # Area Sales Supervisor accounts are scoped to a single region — they can
    # only see/select distributors within their own region, never others.
    "budi":  {"password": "sales123", "full_name": "Budi Santoso", "role": "Area Sales Supervisor", "region": "Jakarta"},
    "wati":  {"password": "sales456", "full_name": "Wati Suryani", "role": "Area Sales Supervisor", "region": "East Java"},
...
def get_combined_progress(distributor_name, period):
    ...
    store = st.session_state.assessment_store.get((distributor_name, period), {})
    total = 0
    items = []
    for q in questions:
        if q in store:
...
def generate_bulk_template(metric_name):
    """Builds an in-memory .xlsx Template sheet (all distributors) + an Info sheet
    documenting the grade/point/description legend.

    Value-based metrics (AR Performance, Data Reporting Compliance): the user types
    a numeric 'value' (days late); grade/point/description auto-fill via formula —
    0 -> A, 1..threshold -> B, >threshold -> C.

    Qualitative metrics (Bank Guarantee / Distributor Manager): the user types the
    'grade' (A/B/C) directly; point/description auto-fill via VLOOKUP.
    """
    master_df = load_master_distributor()
    base_df = master_df[["region", "distributor_code", "distributor"]].rename(
    ...
    with pd.ExcelWriter(buffer, engine="openpyxl") as writer:
        ...
        if is_value_based:
            ...
            for row in range(2, last_row + 1):
                ws[f"E{row}"] = f'=IF(D{row}="","",IF(D{row}<=0,"A",IF(D{row}<={threshold},"B","C")))'
                ws[f"F{row}"] = f'=IF(E{row}="","",IFERROR(VLOOKUP(E{row},Info!$A:$C,3,FALSE),""))'
                ws[f"G{row}"] = f'=IF(E{row}="","",IFERROR(VLOOKUP(E{row},Info!$A:$C,2,FALSE),""))'
            # No lower-bound validation: negative values are valid input — they're
            # clamped to the "0" rule (Grade A) rather than rejected.
            dv = DataValidation(type="whole", operator="greaterThanOrEqual", formula1="-999", allow_blank=True)
            dv.add(f"D2:D{last_row}")
            ws.add_data_validation(dv)
        else:
            for row in range(2, last_row + 1):
                ws[f"E{row}"] = f'=IFERROR(VLOOKUP(D{row},Info!$A:$C,3,FALSE),"")'
                ws[f"F{row}"] = f'=IFERROR(VLOOKUP(D{row},Info!$A:$C,2,FALSE),"")'
...
def get_role_bulk_progress(role, period):
    """For bulk roles: which distributors already have this role's metric
    submitted for the given period, vs still pending."""
    metric = ROLES[role]["questions"][0]
    master_df = load_master_distributor()
    done, pending = [], []
    for _, r in master_df.iterrows():
        key = (r["distributor"], period)
        if metric in st.session_state.assessment_store.get(key, {}):
            done.append(r["distributor"])
        else:
...
# the user picked in the page's date filters at Confirm & Submit time, so
# the dates are guaranteed consistent across the whole batch. MOCK ONLY:
# real app inserts into distributor_sku_allocation (see bq_assessment_schema.sql).
# =====================================================
ALLOCATION_PROGRAMS = {
...
# =====================================================
# LOGIN GATE — role is no longer chosen manually, it's mapped from the
# logged-in account (MOCK_USERS). Real app: BigQuery-backed users table.
# =====================================================
...
# =====================================================
# SESSION STATE INIT
# Mock-only persistence: assessment_store accumulates each role's submitted
# metrics for a given (distributor, period), simulating multiple BigQuery rows
# (one per role) that the real app would aggregate with a query/view. The real
# app must check for an existing (distributor, period, role/metric) row before
# allowing insert, instead of this in-memory session_state.
# =====================================================
if "assessment_store" not in st.session_state:
    st.session_state.assessment_store = {}      # {(distributor, period): {metric: {"points":, "rows":, "submitted_by":}}}
if "role_submitted_keys" not in st.session_state:
    st.session_state.role_submitted_keys = set()  # {(role, distributor, period)}
...
if "pending_bulk_submission" not in st.session_state:
    st.session_state.pending_bulk_submission = None
if "bulk_submission_result" not in st.session_state:
    st.session_state.bulk_submission_result = None
if "just_submitted_bulk" not in st.session_state:
    st.session_state.just_submitted_bulk = False
if "allocation_store" not in st.session_state:
    st.session_state.allocation_store = []   # list of submitted allocation rows (NPD + SKU Focus)
if "pending_allocation" not in st.session_state:
    st.session_state.pending_allocation = None
if "show_alloc_confirm" not in st.session_state:
    st.session_state.show_alloc_confirm = False
if "just_submitted_alloc" not in st.session_state:
    st.session_state.just_submitted_alloc = None   # None, or the program_type just submitted
if "last_alloc_count" not in st.session_state:
    st.session_state.last_alloc_count = 0
...
if not is_bulk_role:
    ...
    # ── METADATA — LOCATION (scoped to the logged-in user's assigned region) ──
    master_df   = load_master_distributor()
    ...
    with st.container(border=True):
        ...
        with c4:
            ...
            region = user_region
        with c5:
            dist_opts = sorted(master_df[master_df["region"] == region]["distributor"].dropna().unique())
    ...
    already_submitted_by_role = role_key in st.session_state.role_submitted_keys
    ...
    with st.sidebar:
        ...
        st.caption("Score updates live as each role fills in & submits their part.")
    ...
    if review_clicked:
        ...
        for q, val in answers.items():
            ...
            base = dict(submission_id=sid, submitted_at=now,
    ...
    def collapse_salesman_for_display(rows):
        """Combine the 5 per-salesman rows into a single summary row so the
        confirmation popup doesn't repeat the same metric/grade/point 5x.
        The actual BigQuery insert rows (one per salesman) are left untouched."""
        display_rows = []
    ...
    def show_confirm_dialog():
        ...
        with b2:
            if st.button("✅  Confirm & Submit", type="primary", use_container_width=True):
                store = st.session_state.assessment_store.setdefault(
                    (data["distributor"], data["assessment_period"]), {}
                )
                for q in data["metrics"]:
                    metric_rows = [r for r in data["rows"] if r["metric"] == q]
                    store[q] = {"points": metric_rows[0]["point"], "rows": metric_rows, "submitted_by": data["role"]}

                st.session_state.role_submitted_keys.add((data["role"], data["distributor"], data["assessment_period"]))
    ...
    if result and (result["role"], result["distributor"], result["assessment_period"]) == role_key:
        ...
        with st.expander("📦 View rows that would be inserted to BigQuery (this submission)", expanded=False):
...
# =====================================================================================
    ...
    # Drop a stale pending bulk confirmation if role/period changed mid-flow
    ...
    if role == "Distributor Manager":
        with st.expander("👤  Create New User (Admin)", expanded=False):
            st.caption("Create a login for any of the 4 roles — no backend/database access needed.")
    ...
    def show_alloc_confirm_dialog():
        ...
        with b2:
            if st.button("✅  Confirm & Submit", type="primary", use_container_width=True, key="alloc_confirm"):
                st.session_state.allocation_store.extend(data["rows"])
    ...
    def render_allocation_section(program_type):
        ...
        with st.expander(f"{cfg['icon']}  {cfg['label']} Allocation Upload", expanded=False):
            ...
            if review_clicked:
                ...
                now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                rows = [{
                    "submission_id": sid, "submitted_at": now,
                    ...
                    "effective_date": eff_date, "end_date": end_date,
                } for r in ready_rows]
            ...
            if st.session_state.just_submitted_alloc == program_type:
                st.success(f"✅ {program_type} allocation submitted — {st.session_state.last_alloc_count} row(s). (MOCK — not sent to BigQuery)")
                st.balloons()
                st.session_state.just_submitted_alloc = False
        ...
        for i, r in df_upload.iterrows():
            ...
            if is_value_based:
                ...
                try:
                    ...
                    row_errors.append(f"Row {i + 2}: invalid value '{raw_value}' for {code} (must be a number of days) — skipped.")
                    continue
                if num_value < 0:
                    num_value = 0  # negative values are clamped to the "0" rule (Grade A)
                ...
                if grade_val not in valid_grades:
                    row_errors.append(
                        f"Row {i + 2}: invalid grade '{grade_val}' for {code} "
                        f"(must be one of {'/'.join(sorted(valid_grades))}) — skipped."
                    )
                    continue

            already = (role, dist_name, assessment_period) in st.session_state.role_submitted_keys
            preview_row = {"distributor_code": code, "distributor_name": dist_name}
            if is_value_based:
    ...
    if bulk_review_clicked:
        ...
        rows = [{
            ...
            "assessment_period": assessment_period,
            "metric": metric_name, "grade": r["grade"],
            "person_name": None, "point": r["points"],
    ...
    def show_bulk_confirm_dialog():
        ...
        with b2:
            if st.button("✅  Confirm & Submit Bulk", type="primary", use_container_width=True, key="bulk_confirm"):
                for r in data["rows"]:
                    key = (r["distributor"], data["assessment_period"])
                    store = st.session_state.assessment_store.setdefault(key, {})
                    store[data["metric"]] = {"points": r["point"], "rows": [r], "submitted_by": data["role"]}
                    st.session_state.role_submitted_keys.add((data["role"], r["distributor"], data["assessment_period"]))

                st.session_state.bulk_submission_result = data
    ...
    if bulk_result and bulk_result["role"] == role and bulk_result["assessment_period"] == assessment_period:
        ...
        with st.expander("📦 View rows that would be inserted to BigQuery (this batch)", expanded=False):

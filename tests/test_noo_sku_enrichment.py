...
def test_write_mode_appends_once_to_the_correct_pool():
    """The write is scoped to the owned span (starting at "NOO/Existing"
    itself, 18 columns as of 2026-09-10 - BD Support removed "asm_name",
    which used to sit between "NOO/Existing" and "input_time", so the span
    now starts one column earlier than "input_time"; see the formula-columns
    tests below), never the full 41-column header - formula and BD-manual
    columns are structurally absent from the payload, not merely blanked."""
    ...
    assert "NOO/Existing" in span_columns  # Streamlit's own column to write
...
def test_owned_span_excludes_every_formula_and_manual_noo_column():
    ...
    assert "NOO/Existing" in touched      # Streamlit's own column, always has been
...
def test_noo_and_not_noo_classification_survive_the_scoped_write_end_to_end():
    """NOO/Existing is Streamlit's own column to write, and always has been
    (see the 2026-09-10 note on config.POOL_NOO_FORMULA_COLUMNS: a live read
    showed nothing else ever populates it - a one-day exclusion was reverted
    the same day). Both it and store_id must land correctly in the sheet."""
    settings = config.Settings(mode="production", env={"WRITE_ENABLED": "true"})

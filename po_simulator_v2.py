...
with st.sidebar:
    ...
    st.divider()
    if st.button("Logout", key="logout_btn_1", use_container_width=True):
                    ...
                    with _btn_col2:
                        if st.button("Auto Hapus SKU", use_container_width=True, key=f"btn_mode_del_{_fi}"):
                            ...
                            if not _parsed:
                                ...
                                _ws_del = next(
                                    (s for s in _wb_del.worksheets if s.title == tpl_selected_sheet),
                                    _wb_del.active,
                                )
                                _hdr_row_del = _tpl_hrow + 1
                                _hdrs_del = {
                                    _ws_del.cell(row=_hdr_row_del, column=c).value: c
    ...
    if _drill_dist and any(d != "(Pilih Distributor)" for d in _drill_dist):
        ...
        if _drill_df.empty:
            ...
            def _highlight_alokasi(val):
                if val == "Reject by Steve":
                    return "background-color: #FFB6C1; color: #8B0000; font-weight: 600;"  # pink mentah
                elif val == "Terdapat Alokasi":
...
with tabs[0]:
    ...
    def export_to_template(
        ...
        _box_border = Border(left=_thin_green, right=_thin_green,
                            top=_thin_green, bottom=_thin_green)
        
        # Row 1: Initiated by, [Distributor]              | APPROVE
        ws.cell(row=sig_row, column=1, value="Initiated by,").font = _black_font
        _cell_init = ws.cell(row=sig_row, column=2, value=f"ASM Approval, ({distributor})")
        _cell_init.font = _green_font
        _cell_init.border = _box_border
        for _c in range(3, 6):
            ws.cell(row=sig_row, column=_c).border = Border(
                top=_thin_green, bottom=_thin_green
            )
        ws.cell(row=sig_row, column=5).border = Border(
            top=_thin_green, bottom=_thin_green, right=_thin_green
        )
        ws.cell(row=sig_row, column=6, value="APPROVE").font = _black_font
        
        # Row 2: (mandatory sign)                          | (SIGN/CAP DISTRIBUTOR)
...
# ── Tulis sheet ke-2 dari Google Sheet ──
    ...
    def excel_to_pdf(
        ...
        elements.append(tbl)
        elements.append(Spacer(1, 15*mm))  # 3 baris kosong
        
        # ── Signature block ──────────────────────────────────────────────
        sig_data = [
            ['Initiated by,', f'ASM Approval, ({distributor})', '', 'APPROVE'],
            ['',              '(mandatory sign)',                '', '(SIGN/CAP DISTRIBUTOR)'],
        ]
        sig_tbl = Table(sig_data, colWidths=[25*mm, 70*mm, 20*mm, 60*mm])
        sig_tbl.setStyle(TableStyle([
            ('FONTNAME',  (0, 0), (-1, -1), 'Trebuchet-Bold'),
            ('FONTSIZE',  (0, 0), (-1, 0), 9),
            ('FONTSIZE',  (0, 1), (-1, 1), 7),
            # Kolom 1 row 0 (ASM Approval) — hijau dengan border
            ('TEXTCOLOR', (1, 0), (1, 0), colors.HexColor('#006400')),
            ('BOX',       (1, 0), (1, 0), 0.8, colors.HexColor('#006400')),
            # APPROVE — hitam, garis bawah hijau
            ('TEXTCOLOR', (3, 0), (3, 0), colors.black),
            ('LINEBELOW', (3, 0), (3, 0), 0.8, colors.HexColor('#006400')),
            # Row 2 — teks merah
        ...
        # Hapus baris elements.append(tbl) di atas karena sudah ada di awal block ini
...
with st.sidebar:
    ...
    st.divider()
    if st.button("Logout", key="logout_btn_2", use_container_width=True):
        st.session_state["authenticated"] = False
        for k in ("raw_df", "data_source", "source_type"):
            st.session_state.pop(k, None)
        st.rerun()
